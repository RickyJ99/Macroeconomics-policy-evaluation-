...
time_series_plot <- function(timeseries) {
    ...
    out <- list(out1, out2, out3, out4, out5, out6)
    return(out)
}
...
```
As we can see from acf its clear signaling the presence of an autcorrelation process. 
...
The results of the ADF tests show the process is not stationary with a $alpha<10%$, so there are no sufficent empirical evidence to reject the null.
